...
def save_best_ckpt(epoch, model, optimizer, scheduler, acc_val, model_fold_name, ckpt_dir):
    print("==> save_best_ckpt called! model type:", type(model))
    model_to_save = model.module if hasattr(model, "module") else model
    print("==> using model_to_save:", type(model_to_save))
    torch.save({
        'epoch': epoch,
        ...
        'acc_val': acc_val
    }, os.path.join(ckpt_dir, f"{model_fold_name}_best.pth"))
...
def get_reduced(val, src_rank, dst_rank, world_size):
    # 非DDP或PG未初始化：直接返回原值
    if not (dist.is_available() and dist.is_initialized() and world_size > 1):
        return val
    device = torch.device(f"cuda:{src_rank}") if torch.cuda.is_available() else torch.device("cpu")
    t = torch.tensor(val, device=device, dtype=torch.float32)
    dist.reduce(t, dst=dst_rank)
    if dist.get_rank() == dst_rank:
        t /= world_size
    # 只有 dst_rank 会拿到均值，其它rank原样返回也行；单进程不进到这里
    return t.item() if dist.get_rank() == dst_rank else val
...
def get_layer_weights(model):
    """
    提取模型中每一层的权重。
    返回一个字典，键是层的名称，值是层的权重。
    """
    layer_weights = {}
    for name, layer in model.named_modules():
        # 如果是线性层或贝叶斯线性层，获取权重
        if isinstance(layer, (nn.Linear, BayesianLinear)):
            layer_weights[name] = layer.weight.detach().cpu().numpy()
